...
// AddMeResponse 添加回复
func AddMeResponse(code int, success bool, message string, id int, token string, time string, ip string, permissions Permissions) MeResponse {
	return MeResponse{
		Code:        code,
		Success:     success,
		Message:     message,
...
func changeMeInfo(id int, name string, password string) MeResponse {
	if id == 0 && Users[id].Name != name {
		return AddMeResponse(403, false, "不可修改根管理员用户名", id, "", "", "", EmptyPermissions)
	}
	Users[id].Name = name
	Users[id].Password = password
	if !SaveInfo(id) {
		return AddMeResponse(404, false, "修改失败", id, "", "", "", EmptyPermissions)
	}
